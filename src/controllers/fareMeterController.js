...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      res.json({ status: true, count: formatted.length, fareMeters: formatted });
    } catch (err) {
...
//     try {
//       const result = await FareMeter.update(req.params.id, req.body);

//       if (result.rowCount === 0)
//         return res.status(404).json({ status: false, message: "Not found" });

//       res.json({ status: true, fareMeter: result.rows[0] });
//     } catch (err) {
//       res.status(500).json({ status: false, message: err.message });
//     }
//   },
