...
const giveGracePeriod = async (req, res) => {
  try {
    ...
    const subscription = await CompanySubscription.getByCompanyId(companyId);
    ...
    const grace = await CompanySubscription.createGrace({
    ...
    companyWebSocket.sendGraceStarted(companyId, grace.grace_until);
    ...
    console.error("Give grace period error:", error);
